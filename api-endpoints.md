...
### GET /api/experts/me
- **Описание**: Получение данных авторизованного эксперта.
- **Авторизация**: Требуется.
- **Пример ответа (успех)**:
  ```json
  {
    ...
    "photo": "https://bluejay-pretty-clearly.ngrok-free.app/storage/experts/ghg1cH30GwveVGIskFz6Mo5LwAl1V81Byt2ShyRu.jpg",
    "experience": "10 лет опыта в google",
    "education": "Гарвард",
    "rating": 0,
    "created_at": "2025-06-09T19:50:31.000000Z",
    "updated_at": "2025-06-09T19:50:31.000000Z",
    "categories": []
  }
  ```
- **Пример ответа (ошибка)**:
...
### GET /api/get-user-future-bookings
- **Описание**: Получение предстоящих записей к текущему эксперту.
- **Авторизация**:
- Требуется (JWT-токен в заголовке `Authorization: Bearer <token>`)
- **Пример ответа (успех)**:
  ```json
...
### GET /api/get-user-completed-bookings
- **Описание**: Получение прошедших записей текущего эксперта.
- **Авторизация**:
- Требуется (JWT-токен в заголовке `Authorization: Bearer <token>`)
- **Пример ответа (успех)**:
  ```json
...
- **Описание**: Выдает все курсы для авторизованного эксперта.
- **Авторизация**: Требуется(роль эксперта).
- **Пример ответа (успех)**:
```json
    {
    "id": 8,
    "expert_id": 10,
    "title": "hi0tgortgob[riogrt]",
    "description": "eroinvbervobvobuv",
    "price": 0,
    "category_id": 1,
    "created_at": "2025-06-09T16:39:42.000000Z",
    "updated_at": "2025-06-09T16:39:42.000000Z",
    "category": {
        "id": 1,
        "title": "Личностный рост",
        "subtitle": "PersonalGrowth",
        "description": "(цели, дисциплина, продуктивность)",
        "position": 1,
        "created_at": "2025-06-08T16:39:46.000000Z",
        "updated_at": "2025-06-08T16:39:46.000000Z"
    }
}
```
- **Пример ответа (ошибка)**:
...
- **Описание**: Создает запись к эксперту(эксперт не может записаться на свой же курс).
- **Параметры**:
    - serviceId (integer)
- **Авторизация**: Есть.
- **Пример ответа (успех)**:
```json
{
    ...
    "expert_name": "Магомед",
    "expert_last_name": "Магомедов",
    "expert_tg_username": "username",
    "expert_phone": "+77098765432",
    "service": "Курс для новых руководителей",
    "service_id": 9
}
```
- **Пример ответа (ошибка)**:
...
- **Описание**: Обновляет запись к эксперту.
- **Параметры**:
    - bookingId (integer)
- **Авторизация**: Есть.
- - **Пример запроса**:
...
- **Описание**: Админ может получить данные отмененных записей в excel-формате.
- **Авторизация**: Есть(и проверка на роль админа).
- ** При успешном ответе админу будет сразу загружаться файл excel-формата статистики.
- **Пример ответа (ошибка)**:
```json
